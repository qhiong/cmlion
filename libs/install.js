...
module.exports = async function installPackage(
  ...
  if (!cuCache && dependencies[pluginName]) {
    console.log(
      `${chalk.yellow(
        "🎵 检测您已经安装过此套件，无须再次安装，如果需要更新套件版本，请执行命令：cmlion update"
      )}`
    );
    return;
  }
  ...
  if (!cuCache) {
    cache.push({
      ...
      desc: targetPlugin
        ...
        : `未知套件, 您可以通过命令 cmlion desc ${pluginName} 来更改它的描述`,
  ...
  try {
    ...
    console.error("🎵 缓存写入失败！请运行命令: cmlion doctor 以修复cmlion-cli");
    process.exit();
  }
  spinner.stop();
  const afterContent = cuCache ? `🎵 同步${pluginName}成功！` : `🎵 安装${pluginName}成功！执行cmlion即可查看您新增的命令!`
